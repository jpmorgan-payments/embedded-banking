...
export const DecisionMakerStepForm = () => {
  ...
  const {
    ...
    mutation: {
      ...
      onSuccess: () => {
        toast.success('Decision maker details updated successfully');
      ...
      onError: () => {
        toast.error('Failed to update decision maker details');
      },
    },
  });
  ...
  const handleEditDecisionMaker = (decisionMakerId: string) => {
    if (getClientStatus === 'success' && clientData) {
      ...
      form.reset(formValues as DecisionMaker);
  ...
  const onSubmit = (values: DecisionMaker) => {
    if (clientId) {
      const requestBody = {
        addParties: [
          {
  ...
  return (
    <div className="eb-space-y-6">
      <h2 className="eb-text-2xl eb-font-bold">Decision Makers</h2>
      <div className="eb-grid eb-grid-cols-1 eb-gap-4 md:eb-grid-cols-2 lg:eb-grid-cols-3">
        {decisionMakers.map((decisionMaker) => (
          <Card key={decisionMaker.id}>
            <CardHeader>
              <CardTitle>{`${decisionMaker?.individualDetails?.firstName} ${decisionMaker?.individualDetails?.lastName}`}</CardTitle>
            </CardHeader>
            <CardContent>
              <p>{decisionMaker?.individualDetails?.jobTitle ?? ''}</p>
              <div className="eb-mt-4 eb-space-x-2">
                <Button
                  ...
                  onClick={() =>
                    decisionMaker?.id &&
                    handleEditDecisionMaker(decisionMaker?.id)
                  }
                >
                ...
                <Button
                  ...
                  onClick={() =>
                    decisionMaker?.id &&
                    handleDeleteDecisionMaker(decisionMaker?.id)
                  }
                >
        ...
        >
          <CardContent className="eb-flex eb-items-center eb-justify-center">
            ...
            <p className="eb-mt-2">Add Decision Maker</p>
          </CardContent>
        </Card>
      </div>
